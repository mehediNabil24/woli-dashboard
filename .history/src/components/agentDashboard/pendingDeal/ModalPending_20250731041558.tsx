...
export default function EditDealModal({ visible, onCancel, initialData }: EditDealModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    e.preventDefault()
    try {
      const res = await updateDeal({ id: initialData?.key, updateData: formData }).unwrap()
      if (res?.success) {
  ...
  return (
    ...
    >
      <div className="p-6">
        <form action="" onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
          {/* State */}
          <div>
