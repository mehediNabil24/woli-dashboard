...
const [updateDeal]=useUpdateDealsMutation()
  ...
  const handleUpdate =async(id:any)=>{
    const updateData={
      ...
      clientFirstName: defaultData.clientFirstName,
      clientLastName: defaultData.clientLastName,
      applicationNumber: defaultData.applicationNumber,
      annualPremium: defaultData.annualPremium,
      note: defaultData.note,
    }
    
   try{
    const res=await updateDeal({id,updateData}).unwrap()
    if(res?.success){
      ...
      <div className="p-6">
        <form className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
          ...
          <div className="col-span-full flex justify-center mt-6">
            <Button onClick={()=>handleUpdate(initialData?.key)}
              type="primary"
              htmlType="submit"
              style={{
              backgroundColor: "#000",
